...
out_folder = '/path/to/fit/results/' #folder where all outputs are stored

priorcsv = str(sys.argv[1])           #name of prior csv

#loading in data from csv file
data = pd.read_csv(filepath+'KOI134_seg.csv',comment='#') #reading in light curve file
ntransit_vs_center = pd.read_csv(filepath+'transittime1.csv', comment='#',delim_whitespace=True)  #csv of approximate transit times and durations
...
BPmag = 13.92255
BPmagerr = .03                       #error floor of .03 for gaia bands
...
def sed_likelihood(params):
    ...
    if mist_rad<0.5 or mist_rad>2.5:
        return -np.inf
...
if __name__ == '__main__':
    ...
    system_list, freeparams, fixed_params, true_values = read_fit_param_csv(filepath+priorcsv)
    ...
    with Pool(processes=60) as pool:
        if output == 'h5':
            ...
            os.system("rm -rf "+out_folder+"mcmc_tested_params")
            os.system("rm -rf "+out_folder+ "testparams_step")

            header2 = freeparams.unpack()
            header2.append('likelihood')
            header = np.array(header2)

            np.savetxt(out_folder+'testparams_step', header.reshape(1,header.shape[0]),fmt='%s')
            os.system('cat '+out_folder+'testparams_step >> '+ out_folder+'mcmc_tested_params')
            ...
            for sample in sampler.sample(pos, iterations=max_n, store=False):
                ...
                for i in range(len(position)):
                    ...
                    if counter >= 100 * nwalkers:
                        np.savetxt(out_folder+"testparams_step", master_pos, fmt='%.10f')
                        os.system("cat "+ out_folder +"testparams_step >> " + out_folder + "mcmc_tested_params")
                        counter = 0
